...
export const orderValidateSchema = z.object({
    ...
    productId: objectIdSchema.refine(data => data, { message: "Product id is required." }),
    price: z.number({ required_error: "Price is required." }).nonnegative({ message: "Price must be a non-negative number" }),
    quantity: z.number({ required_error: "Quantity is required." }).int({ message: "Quantity must be a number." }).positive({ message: "Quantity must be a positive integer" }),
});