...
const creatOrder = async (orderData: TOrder) => {
    ...
    const availableQuantity = product?.inventory?.quantity as number - orderData.quantity
    ...
    if (availableQuantity >= 0 && product?.inventory?.inStock) {
        ...
        if (result) {
            ...
            await Product.updateOne({ _id: orderData.productId }, { $inc: { "inventory.quantity": -orderData.quantity }, "inventory.inStock": availableQuantity > 0 ? true : false })
        }
        return result
    }
}
